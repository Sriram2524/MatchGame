...
class App extends Component {
  ...
  clickThumbnail = id => {
    const {currentImageId, score} = this.state
    if (currentImageId !== id) {
      clearInterval(this.timerId)
      this.setState({gameOver: true})
    } else {
      const index = Math.floor(Math.random() * imagesList.length)
      const crntImgId = imagesList[index].id
      this.setState({score: score + 1, currentImageId: crntImgId})
    }
  }
  ...
  render() {
    const {score, activeTabId, timer, currentImageId, gameOver} = this.state
    const filteredList = this.getFilteredList()
    return (
      <div className="bg-container">
        <Header timer={timer} score={score} />
        <div className="game-view-con">
          {gameOver ? (
            <GameOver resetButton={this.resetButton} score={score} />
          ) : (
            <div className="game-false-page">
              <CurrentImage
              ...
              <ul className="unordered-tabList">
                {tabsList.map(each => (
                  <TabsList
                    ...
                    active={each.tabId === activeTabId}
                    eachTabDetails={each}
                    key={each.tabId}
                  />
                ))}
              </ul>
              <ul className="unordered-tumbnailList">
                {filteredList.map(each => (
                  <ThumbnailList
                    key={each.id}
                    clickThumbnail={this.clickThumbnail}
                    eachTumbnailDetails={each}
                  />
                ))}
              </ul>
            </div>
